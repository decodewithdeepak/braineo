...
const Flashcards = () => {
  ...
  const [generatingCards, setGeneratingCards] = useState(false); // Separate state for card generation
  ...
  useEffect(() => {
    const fetchPaths = async () => {
      try {
        setLoading(true); // Show loading screen while fetching paths
        ...
        setLoading(false); // Hide loading when fetch completes
      }
    };

    if (user?.$id) fetchPaths();
    else setLoading(false); // If no user, don't show loading
  }, [user]);
  ...
  const fetchFlashcards = async () => {
    ...
    setGeneratingCards(true); // Use this state for generating cards
    setError(null);
    ...
    try {
      ...
      setGeneratingCards(false); // Reset card generation state
    }
  };
    ...
    >
      {loading ? (
        ...
        0L3.34 16c-.77 1.333.192 3 1.732 3z"
          ...
          {cards.length > 0 && flashcardNudges.length > 0 && (
            <motion.div
